...
<div id="auth-box" style="margin-bottom: 20px; text-align: center;">
  <input type="password" id="auth-token" placeholder="Enter security token" style="background:#0a1f35; color:#fff; border:1px solid #1a3f5c; padding:8px; border-radius:4px;">
  <button onclick="refresh()" style="background:#00ffe0; border:none; padding:8px 16px; border-radius:4px; font-weight:bold; cursor:pointer;">UNLOCK</button>
</div>
...
async function refresh(){
  ...
  try{
    ...
    if (r.status === 401) { alert("Invalid security token"); return; }
...
setInterval(() => {
    if (document.getElementById('main-content').style.display === 'block') refresh();
}, 5000);
</script>
</body>
</html>'''
...
class _Handler(BaseHTTPRequestHandler):
    ...
    def _check_auth(self):
        # Allow / and /index.html without token to show login box
    ...
    def do_POST(self):
        # Accept sync data from mobile companion - check token
        if not self._check_auth():
            self._send(401, 'application/json', b'{"error": "Unauthorized"}')
            return
...
class WebMonitorScreen(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        self._token_lbl = ctk.CTkLabel(
            self._sc, text=f"Security Token: {get_auth_token()}",
            font=MONO_SM, text_color=C['ok'])
        self._token_lbl.pack(pady=(4, 10))

        ctk.CTkLabel(self._sc,
            text='Open this URL on any device on the same network.\n'
                 'Enter the security token above when prompted.',
        ...
        for ep, desc in [
            ...
            ('/api/data    ', 'JSON data endpoint — score, ports, system (Requires Token)'),
